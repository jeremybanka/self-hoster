...
# allow http, https, and dns (tcp) in
iptables -A INPUT -p tcp -m multiport --dports 80,443,53 -j ACCEPT
# allow dns (udp) in
iptables -A INPUT -p udp --dport 53 -j ACCEPT
# allow ssh on 22 and 2222 in
...
# allow http, https, and dns (tcp) out
...
# port scanning drop (changed to be slightly more permissive)
